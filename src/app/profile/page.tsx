...
import { MOCK_PROFILE, MOCK_SAVED_ITEMS, MOCK_HOTEL_REVIEWS } from "@/lib/mock-data";
import { formatBDT } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { User, Ticket, Bookmark, Star, Edit3, Save, Printer, ShieldCheck, Mail, Phone, Lock, Heart, CheckCircle2 } from "lucide-react";
      ...
      <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-900 text-white font-extrabold text-2xl shadow-sm">
            {profile.display_name?.slice(0, 2).toUpperCase() || "MR"}
          </div>
          <div>
            <span className="text-xs font-bold text-emerald-700 uppercase tracking-wider block">Member Profile</span>
            <h1 className="font-outfit text-2xl sm:text-3xl font-black text-slate-900">{profile.display_name}</h1>
        ...
        <div className="flex items-center gap-2 bg-white p-3 rounded-2xl border border-slate-200 text-xs font-semibold">
          ...
          <div>
            <p className="font-bold text-slate-900">Verified Traveller</p>
            <p className="text-slate-500">NID: {profile.national_id}</p>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleProfileSave} className="p-6 rounded-3xl border border-slate-200 bg-white space-y-6 shadow-xs">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                {!isEditing ? (
                  ...
                  <Button type="submit" size="sm" className="gap-1.5 font-bold text-xs">
            ...
            {bookings.map((booking) => (
              <div key={booking.id} className="p-5 rounded-2xl border border-slate-200 bg-white space-y-3 shadow-xs">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
                  ...
                  <Button size="sm" variant="outline" onClick={() => window.print()} className="font-bold gap-1 rounded-xl">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl border border-slate-200 bg-white space-y-2">
                <Badge variant="outline" className="text-[10px] font-bold">HOTEL</Badge>
                <h4 className="font-bold text-slate-900 text-sm">Ocean Paradise Hotel & Resort</h4>
                <p className="text-xs text-slate-500">Cox's Bazar • 5 Star Luxury Resort</p>
                <Button size="sm" className="w-full font-bold text-xs mt-2">View Hotel</Button>
              </div>

              <div className="p-4 rounded-2xl border border-slate-200 bg-white space-y-2">
                <Badge variant="outline" className="text-[10px] font-bold">TOUR</Badge>
                <h4 className="font-bold text-slate-900 text-sm">Cox's Bazar Beach & Inani Sunset Adventure</h4>
                <p className="text-xs text-slate-500">3 Days / 2 Nights • BDT 8,500</p>
                <Button size="sm" className="w-full font-bold text-xs mt-2">View Tour</Button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {reviews.map((rev) => (
              <div key={rev.id} className="p-4 rounded-2xl border border-slate-200 bg-white space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-xs text-slate-900">Ocean Paradise Resort</span>
